...
<body>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div>
            <h2 class="fw-bold mb-1 text-dark">Chỉnh Sửa Danh Mục</h2>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Chỉnh sửa #${category.categoryId}</li>
                </ol>
            </nav>
        </div>
        ...
        <div class="col-12 col-lg-8">
            <div class="card-custom">
                <div class="card-custom-header">
                    <h5 class="card-custom-title"><i class="fa-solid fa-pen-to-square text-primary me-2"></i>Cập Nhật Danh Mục #${category.categoryId}</h5>
                </div>
                <div class="card-custom-body">
                    <form action="${pageContext.request.contextPath}/admin/category/update" method="post" enctype="multipart/form-data">
                        
                        <!-- ID và Current Image ẩn -->
                        <input type="hidden" name="categoryId" value="${category.categoryId}" />
                        <input type="hidden" name="currentImage" value="${category.images}" />
                            ...
                            <div class="d-flex align-items-center gap-4 flex-wrap">
                                <div class="preview-box" id="previewContainer">
                                    <c:choose>
                                        <c:when test="${not empty category.images}">
                                            <img src="${category.images.startsWith('http') ? category.images : pageContext.request.contextPath.concat('/uploads/').concat(category.images)}" 
                                                 id="imagePreview" alt="${category.categoryname}">
                                            <span class="text-muted text-center small p-2 d-none" id="placeholderText">Chưa có ảnh</span>
                                        </c:when>
                                        <c:otherwise>
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="form-text text-muted mt-2">
                                        Chọn ảnh mới nếu muốn thay thế ảnh hiện tại. Để trống nếu giữ nguyên ảnh cũ.
                                    </div>
                                </div>
                            </div>
                        </div>
